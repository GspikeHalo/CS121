...
class InvertedIndexProcessor:
    ...
    def init_inverted_index(self, db) -> None:
        ...
        sql = "CREATE TABLE IF NOT EXISTS inverted_index (token TEXT, doc_id TEXT, word_num INT, FOREIGN KEY(token) REFERENCES tokens(token), FOREIGN KEY(doc_id) REFERENCES documents(doc_id))"
        self._db.execute(sql)
        self._db.commit()

    def update_inverted_index(self, token_weight: dict, doc_id: str) -> None:
        sql = "INSERT INTO inverted_index (token, doc_id, word_num) VALUES (?, ?, ?)"

        for token, word_num in token_weight.items():
            self._cursor.execute(sql, (token, doc_id, word_num))
        self._db.commit()

    def remove_duplicate(self, doc_ids: list):
        sql = "DELETE FROM inverted_index WHERE doc_id = ?"
